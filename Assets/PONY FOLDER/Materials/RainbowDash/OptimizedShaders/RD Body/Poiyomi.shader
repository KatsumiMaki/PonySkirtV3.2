Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/e59256be84129234fb792788d69d6b47"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0.6172066,0.8631573,1,1) * lerp(indirectColor, 1, float(0.038))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.509));
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0.6172066,0.8631573,1,1), float4(0.6172066,0.8631573,1,1), float(0.038)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(0.6172066,0.8631573,1,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.038));
