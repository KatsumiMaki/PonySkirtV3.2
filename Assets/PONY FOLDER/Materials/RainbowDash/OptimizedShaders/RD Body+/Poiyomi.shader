Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/52bcc7144bfa7fa46a79a08a2f6ef572"
{
	Properties
	{
 ...
 #define PROP_CLIPPINGMASK 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0.6150811,0.8555073,1,1) * lerp(indirectColor, 1, float(0.038))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.509));
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0.6150811,0.8555073,1,1), float4(0.6150811,0.8555073,1,1), float(0.038)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(0.6150811,0.8555073,1,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.038));
