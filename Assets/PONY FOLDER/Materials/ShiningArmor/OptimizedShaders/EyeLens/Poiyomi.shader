Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/93e492398131bad418b2ad716f8098ef"
{
	Properties
	{
 ...
 #define PROP_MATCAP 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(-0.85));
			}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				switch(float(1))
				{
					case 0:
					{
						...
						matcapUV = noSknewViewNormal.rg * float(0.314) + 0.5;
						break;
					}
					case 1:
					{
						...
						matcapUV = float2(dot(worldViewRight, normal0), dot(worldViewUp, normal0)) * float(0.314) + 0.5;
						break;
					}
					case 2:
					{
				...
				blendMatcap(poiLight, poiFragData, float(0), float(0), float(0), float(1), float(0), matcap, matcapMask, float(0), float(1)
				#ifdef POI_BLACKLIGHT
