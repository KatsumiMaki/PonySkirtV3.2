Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/79cb17b56e3c17d4e95e84af090cdc88"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,0.8181564,0.5832484,1) * lerp(indirectColor, 1, float(0.038))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.509));
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(1,0.8181564,0.5832484,1), float4(1,0.8181564,0.5832484,1), float(0.038)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(1,0.8181564,0.5832484,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.038));
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.05951124,0.3611521,1).rgb, float(0)), float4(1,0.05951124,0.3611521,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.05951124,0.3611521,1).rgb, float(0)), float4(1,0.05951124,0.3611521,1).a);
				#endif
				...
				rim *= float4(1,0.05951124,0.3611521,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.05951124,0.3611521,1).rgb, float(0)));
