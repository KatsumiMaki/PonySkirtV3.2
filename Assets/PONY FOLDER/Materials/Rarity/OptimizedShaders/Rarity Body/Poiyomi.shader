Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/ce44f78aa1eb92f41a18576e63a0fbd1"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0.9471843,0.9816728,1,1) * lerp(indirectColor, 1, float(0.038))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.509));
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0.9471843,0.9816728,1,1), float4(0.9471843,0.9816728,1,1), float(0.038)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(0.9471843,0.9816728,1,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.038));
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.04231141,0.5332766,0.7454045,1).rgb, float(0)), float4(0.04231141,0.5332766,0.7454045,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.04231141,0.5332766,0.7454045,1).rgb, float(0)), float4(0.04231141,0.5332766,0.7454045,1).a);
				#endif
				...
				rim *= float4(0.04231141,0.5332766,0.7454045,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.04231141,0.5332766,0.7454045,1).rgb, float(0)));
