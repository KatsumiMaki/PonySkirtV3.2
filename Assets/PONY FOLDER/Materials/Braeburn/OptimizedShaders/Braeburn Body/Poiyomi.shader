Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/827360d273b19844dbbd6fda6b1655c6"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.9162945,0.005763467,0.0172622,1).rgb, float(0)), float4(0.9162945,0.005763467,0.0172622,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.9162945,0.005763467,0.0172622,1).rgb, float(0)), float4(0.9162945,0.005763467,0.0172622,1).a);
				#endif
				...
				rim *= float4(0.9162945,0.005763467,0.0172622,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.9162945,0.005763467,0.0172622,1).rgb, float(0)));
