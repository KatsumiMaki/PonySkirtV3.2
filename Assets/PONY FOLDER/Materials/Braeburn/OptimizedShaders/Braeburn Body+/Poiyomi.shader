Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/0340efd74b3c5c54ca55b6ceb66ac8d1"
{
	Properties
	{
 ...
 #define PROP_CLIPPINGMASK 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.038));
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,0.8860005,0.7343559,1) * lerp(indirectColor, 1, float(0.038))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.509));
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(1,0.8860005,0.7343559,1), float4(1,0.8860005,0.7343559,1), float(0.038)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				if (float(0) == 1)
				{
					...
					#endif
					if (float(0.655) == float(0)) float(0.655) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.655), float(0), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.655), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.655), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(0) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.655), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(0.61));
				if (float(0) && float(0))
				{
				...
				float blendStrength = float(0.333);
				#ifdef POI_AUDIOLINK
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.9162945,0.005763464,0.01726219,1).rgb, float(0)), float4(0.9162945,0.005763464,0.01726219,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.9162945,0.005763464,0.01726219,1).rgb, float(0)), float4(0.9162945,0.005763464,0.01726219,1).a);
				#endif
				...
				rim *= float4(0.9162945,0.005763464,0.01726219,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.9162945,0.005763464,0.01726219,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0))));
