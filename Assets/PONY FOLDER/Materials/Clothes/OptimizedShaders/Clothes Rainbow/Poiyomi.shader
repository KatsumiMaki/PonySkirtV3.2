Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/c0c794ce20819e345b614dd3dd865cd1"
{
	Properties
	{
 ...
 #define PROP_HIGHCOLOR_TEX 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float reflectance = float(0.744) * clothmapsample.b;
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(2.29));
				if (float(0) && float(0))
				{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0.1335305,0.3377189,1,1).rgb, float(0)), float4(0.1335305,0.3377189,1,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(0.1335305,0.3377189,1,1).rgb, float(0)), float4(0.1335305,0.3377189,1,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.415), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(0.1335305,0.3377189,1,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(0.1335305,0.3377189,1,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.238))));
			...
			void stylizedSpecular(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					specMask1 += pow(specArea, exp2(lerp(11, 1, float(0.488)))) * float(0.079);
					specMask2 += pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0);
				}
				else
				{
					specMask1 += poiEdgeNonLinear(specArea, (1.0 - pow(float(0.488), 5)), float(0.374)) * float(0.079);
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0))
					{
						vSpecMask = pow(specArea, exp2(lerp(11, 1, float(0.488)))) * float(0.079) * poiLight.vAttenuation * poiLight.vColor[index];
						vSpecMask = max(vSpecMask, pow(specArea, exp2(lerp(11, 1, float(0)))) * float(0) * poiLight.vAttenuation * poiLight.vColor[index]);
					}
					else
					{
						vSpecMask = poiEdgeNonLinear(specArea, (1.0 - pow(float(0.488), 5)), float(0.374)) * float(0.079) * poiLight.vAttenuation * poiLight.vColor[index];
